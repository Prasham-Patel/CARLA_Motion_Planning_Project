...
if __name__ == '__main__':
    ...
    # VEHICLE 1 SPAWN
    spawn_point1 = carla.Transform(carla.Location(x=-24, y=135, z=0.3), carla.Rotation(yaw=0))
    ...
    # map = world.get_map()
    goal = CARLA_world.map.get_waypoint(carla.Location(x=2.3, y=120), project_to_road=True)
    print(goal)
    ...
    for i in range(len(path_x) - 1):
        ...
        while ((wheel_F_x - w_x2) ** 2 + (wheel_F_y - w_y2) ** 2) ** 0.5 >= 0.5:
            control = controller.run_step(RRT_planner.v*3.6)
